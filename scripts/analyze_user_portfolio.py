...
def main():
    ...
    for idx, (_, row) in enumerate(contrib_df.iterrows(), 1):
        print(f"{idx:<6} {row['risk_id']:<8} ${row['mean_loss']:>14,.0f} "
    ...
    for _, row in tornado_df.iterrows():
        print(f"{row['risk_id']:<8} {row['category']:<15} "
    ...
    print(f"   → Budget this amount for risk reserves")
    print()
    print(f"2. Worst Case Planning (95% VaR): ${stats['var_95']:,.0f}")
    print(f"   → 1-in-20 year loss scenario")
    print(f"   → Consider capital allocation or insurance above this level")
    print()
    print(f"3. Extreme Tail Risk (99% TVaR): ${stats['tvar_99']:,.0f}")
    print(f"   → Average loss in worst 1% of scenarios")
    print(f"   → Regulatory capital requirement benchmark")
    print()
    print(f"4. Top Risk Driver: {top_risk['risk_id']} ({top_risk['contribution_pct']:.1f}% of expected loss)")
    print(f"   → Category: {register_df[register_df['RiskID']==top_risk['risk_id']]['Category'].values[0]}")
    print(f"   → Prioritize mitigation efforts here")
    print()
    print(f"5. Top Tail Risk Driver: {top_tail_risk['risk_id']}")
    print(f"   → Drives worst-case scenarios (dVaR: ${top_tail_risk['dvar']:,.0f})")
    print(f"   → Focus on extreme risk reduction")
    print()
    ...
    fig_tornado_dvar = plot_tornado(
        tornado_df,
        metric="dvar",
        title="Top Risk Contributors by Tail Risk (dVaR)"
    )
