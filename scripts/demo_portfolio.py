...
def main():
    ...
    print("-" * 70)
    print(
        register_df[["RiskID", "Category", "FrequencyModel", "SeverityModel"]].to_string(
            index=False
        )
    )
    print()
    ...
    for _, row in lec_df.iterrows():
        ...
        print(
            f"  {prob_pct:5.1f}% chance of loss ≥ ${loss_val:>15,.0f}  (Return period: {return_period:>6.1f} years)"
        )
    print()
    ...
    fig_tornado_dvar = plot_tornado(
        tornado_df, metric="dvar", title="Top Risk Contributors by Tail Contribution (dVaR)"
    )
    ...
    print(
        f"  • Top risk contributor: {contrib_df.iloc[0]['risk_id']} ({contrib_df.iloc[0]['contribution_pct']:.1f}% of expected loss)"
    )
