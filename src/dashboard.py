...
def display_risk_register_tab():
    ...
    # Add risk zones
    fig.add_shape(type="rect", x0=0, y0=0.7, x1=df['impact'].max()*0.3, y1=1.0,
                  fillcolor="yellow", opacity=0.2, line_width=0)
    fig.add_shape(type="rect", x0=df['impact'].max()*0.7, y0=0.7, x1=df['impact'].max(), y1=1.0,
    ...
    with col3:
        sort_by = st.selectbox("Sort by", ['residual_risk_score', 'inherent_risk_score', 'impact', 'likelihood'])
    ...
    st.dataframe(
        filtered_df[[
            ...
            'inherent_risk_score', 'residual_risk_score', 'owner', 'status'
        ]].style.format({
            'likelihood': '{:.2%}',
            'impact': '${:,.0f}',
...
def display_monte_carlo_tab():
    ...
    with col2:
        ...
        category_losses = results_with_cat.groupby('category')['mean_loss'].sum().sort_values(ascending=False)
    ...
    display_df = results[[
        'risk_id', 'risk_name', 'category', 'mean_loss', 'median_loss',
        'p95_loss', 'p99_loss', 'max_loss'
    ...
    fig.update_layout(
        title="Top 5 Risk Contributors to Portfolio Loss",
        showlegend=False,
        height=400
    )
...
def display_lec_tab():
    ...
    var_cvar = lec.get_var_cvar(
        st.session_state.portfolio_stats['all_simulations'],
...
def display_kpi_dashboard():
    ...
    with col2:
        ...
        category_comparison = df.groupby('category').agg({
            'inherent_risk_score': 'sum',
            'residual_risk_score': 'sum'
        }).reset_index()
    ...
    trend_data = pd.DataFrame({
        'Month': months,
        'Inherent Risk': [df['inherent_risk_score'].sum() * (1 + np.random.uniform(-0.1, 0.1)) for _ in range(6)],
        'Residual Risk': [df['residual_risk_score'].sum() * (1 + np.random.uniform(-0.1, 0.1)) for _ in range(6)]
    })
