...
def _coerce_types(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Numeric columns
    numeric_cols = [
        "FreqParam1", "FreqParam2", "SevParam1", "SevParam2", "SevParam3",
        "ControlEffectiveness", "ResidualFactor"
    ]
...
def _validate_register(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "ControlEffectiveness" in df.columns:
        ...
        if len(invalid) > 0:
            errors.append(f"ControlEffectiveness out of range [0,1] in rows: {invalid.index.tolist()}")
...
def validate_register_format(df: pd.DataFrame) -> tuple[bool, List[str]]:
    ...
    required_cols = ["RiskID", "FrequencyModel", "FreqParam1", "SeverityModel", "SevParam1", "SevParam2"]
