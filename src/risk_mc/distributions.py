...
def sample_frequency_poisson(
    lam: float, n_sims: int, rng: Optional[np.random.Generator] = None
) -> np.ndarray:
    """
...
def sample_frequency_negbin(
    r: float, p: float, n_sims: int, rng: Optional[np.random.Generator] = None
) -> np.ndarray:
    """
...
def sample_severity_lognormal(
    mu: float, sigma: float, n_events: int, rng: Optional[np.random.Generator] = None
) -> np.ndarray:
    """
