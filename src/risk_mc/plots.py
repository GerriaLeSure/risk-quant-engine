...
def risk_contribution_chart(
    ...
    ax.set_title(
        f"Top {top_n} Risks by {metric.replace('_', ' ').title()}", fontsize=14, fontweight="bold"
    )
...
def frequency_severity_scatter(
    register_df: pd.DataFrame, portfolio_df: pd.DataFrame, figsize: tuple[int, int] = (10, 8)
) -> plt.Figure:
    """
...
def compare_distributions(
    distributions: dict, title: str = "Distribution Comparison", figsize: tuple[int, int] = (12, 6)
) -> plt.Figure:
    """
...
def plot_dual_tornado(tornado_df: pd.DataFrame, figsize: tuple[int, int] = (14, 8)) -> plt.Figure:
    """
